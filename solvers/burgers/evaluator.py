...
def save_visualization(u_batch_np: np.array, u_ref_np: np.array, save_file_idx=0):
    ...
    plt.savefig(os.path.join(args.save_pth, f'burgers_visualization_{save_file_idx}.png'))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script for solving 1D Burgers' Equation.")
    ...
    parser.add_argument("--nu", type=float,
                        default=1.0,
                        choices=[1.0, 2.0, 4.0, 0.1, 0.2, 0.4, 0.01, 0.02, 0.04, 0.001, 0.002, 0.004],
                        help="The viscosity coefficient.")
    parser.add_argument("--dataset-path-for-eval", type=str,
                        default='/usr1/username/data/CodePDE/Burgers/1D_Burgers_Sols_Nu1.0.hdf5',
    ...
    u_batch = solver(u0, t_coordinate, nu)
    end_time = time.time()
    print(f"##### Finished #####")
